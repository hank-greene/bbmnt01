...
public class DefaultTableRenderer implements ITableRenderer, Serializable {
	...
	public void renderTable(RenderContext renderContext, TableViewer viewer, TableModel model,
		...
		// writer.print("<DIV id=\"tblViewResizer_" + viewer.getControlID() +
		// "\" class=\"tblViewResizer\" ");
		// /*writer.print("onMouseUp=\"tblViewer_resizeColumnDone()\"" +
		// " onMouseMove=\"tblViewer_resizeColumMove()\"");*/
		// writer.print(" style=\"height: " + (viewer.getHeight() != 0 ?
		// viewer.getHeight() : 20) + "px\"");
		// writer.println("></DIV>");
		// }
		...
		if (viewer.isFillWidth()) {
			...
			/*
			 * if (viewer.getHeight() != 0) { writer.print(" height=\"" +
			 * viewer.getHeight() + "\""); }
			 */
		}
		writer.println(">");
		...
		String divHeight = viewer.getHeight() != 0
				? (viewer.isShowStatusBar() ? viewer.getHeight() - 18 : viewer.getHeight()) + "px" : "100%";
		...
		if (viewer.isShowHeader() || !viewer.isScrollable()) {
			...
			if (viewer.isScrollable()) {
				// must add a width attribute, otherwise table-layout: fixed
				// isnt working on Mozilla
		...
		if (viewer.isScrollable()) {
			...
			writer.print("<DIV onscroll=\"JWic.controls.TableViewer.handleScroll(event, '" + viewer.getControlID()
					+ "')\" style=\"");
		...
		try {
			...
			System.out.println("Error reading data from ContentProvider"+e.toString());
		}
		...
		if (viewer.isScrollable()) {
			...
			writer.println(
					"<INPUT TYPE=\"HIDDEN\" NAME=\"" + fldLeft.getId() + "\" VALUE=\"" + fldLeft.getValue() + "\">");
			writer.println(
					"<INPUT TYPE=\"HIDDEN\" NAME=\"" + fldTop.getId() + "\" VALUE=\"" + fldTop.getValue() + "\">");

			writer.println("<script language=\"javascript\">");
			writer.println("window.setTimeout(\"JWic.restoreScrolling('" + viewer.getControlID()
					+ "', 'tblViewDataLayer_" + viewer.getControlID() + "');\", 0);");
	...
	protected int renderRows(int level, boolean hasNext, PrintWriter writer, Iterator<?> it, TableViewer viewer,
			ITableLabelProvider labelProvider) {
				...
				if (column.getIndex() == viewer.getExpandableColumn()) {
					...
					if (contentProvider.hasChildren(row)) {
						if (viewer.isEnabled()) {
							...
							writer.print(expanded ? "JWic.controls.TableViewer.collapse(event)"
									: "JWic.controls.TableViewer.expand(event)");
							writer.print("\";return false;\">");
							writer.print((expanded ? ICON_COLLAPSE : ICON_EXPAND).toImgTag(expandIconWidth,
									expandIconHeight));
							writer.print("</A>");
						} else {
							writer.print((expanded ? ICON_COLLAPSE : ICON_EXPAND).toImgTag(expandIconWidth,
	...
	/**
	 * @param expandLinkSpacing
	 *            the expandLinkSpacing to set
	 */
	...
	/**
	 * @param expandIconHeight
	 *            the expandIconHeight to set
	 */
	...
	/**
	 * @param expandIconWidth
	 *            the expandIconWidth to set
	 */
